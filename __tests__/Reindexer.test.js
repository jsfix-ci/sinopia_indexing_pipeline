  ...
  describe('reindex()', () => {
    ...
    describe('when indexing fails', () => {
      ...
      it('calls the indexer and logs an error', async () => {
        ...
        await reindexer.reindex()
        expect(indexerSpy).toHaveBeenCalledTimes(2)
        expect(errorSpy).toHaveBeenNthCalledWith(1, `error reindexing ${fakeResource1.uri}: ElasticSearch is down`)
        expect(errorSpy).toHaveBeenNthCalledWith(2, `error reindexing ${fakeResource2.uri}: ElasticSearch is down`)
      })
    })
  })
})
