...
export default class Indexer {
  ...
  async index(doc) {
    ...
    try {
      ...
      this.logger.error(
        `Could not load dataset for ${doc.uri}: ${err}. ${JSON.stringify(doc)}`
      )
      return null
    }
    ...
    if (!resourceType) {
      this.logger.error(
        `Could not determine resource type for ${doc.uri}: ${JSON.stringify(
          doc
        )}`
      )
      return null
    ...
    this.logger.debug(
      `Indexing ${
        doc.uri
      } (${resourceType}) into index ${index}: ${JSON.stringify(body)}`
    )
      ...
      for (const resourceType of Object.keys(this.indexers)) {
        ...
        if (!indexExists.body) {
          ...
          await this.client.indices.create({
            index: index,
            body: { settings: this.indexSettings() },
          })
        }
  ...
  resourceTypeFor(dataset, uri) {
    ...
    return resourceClass === "http://sinopia.io/vocabulary/ResourceTemplate"
      ? "template"
      : "resource"
  }
}
